...
const InPlayView: React.FC = () => {
  ...
  const who_started_this_round =
    ((game.whos_turn ?? 0) - game.floor_cards.length + 4) % 4;
      ...
      {game.your_index >= 0 ? (
        <Box>
          <Alert severity="info">
            Welcome
            <Player
              name={game.players[game.your_index]}
              index={game.your_index}
              {...playerProps}
            />
          </Alert>
        </Box>
        ...
        <Box>
          <Alert severity="info">
            This game already has all it's players — you can still watch it
          </Alert>
        </Box>
      )}
      <Box sx={{ py: 2 }}>
        ...
        <Grid container spacing={2} textAlign="center" sx={{ pb: 8 }}>
          ...
          <Grid item xs={12} md={4}>
            <Player
              name="Team A"
              index={0 as PlayerIndex}
              score={game.wins[0]}
            />
            {game.scoreline.join(" — ")}
            <Player
              name="Team B"
              index={1 as PlayerIndex}
              score={game.wins[1]}
            />
          </Grid>
        </Grid>
        ...
        <Box sx={{ textAlign: "center", mt: -1.7 }}>
          {[0, 1, 2, 3].map((suite) => (
            <CardSuite
              suite={suite as PlayingCardSuite}
              highlighted={game.trump_suite}
            />
          ))}
        </Box>
        ...
        <Box sx={{ textAlign: "center", py: 2 }}>
          <CardSet
            cards={game.your_cards}
            sort={true}
            onSelectFactory={(card) =>
              canPlayCard(card) ? () => api.send({ card }) : null
            }
            ...
            {[0, 1, 2, 3].map((ix) => (
              <Box display="inline-block">
                <Box flexDirection="row">
                  <Box>
                    <Player
                      name={game.players[(who_started_this_round + ix) % 4]}
                      index={((who_started_this_round + ix) % 4) as PlayerIndex}
                      {...playerProps}
                    />
                  </Box>
                  <Box>
        ...
        {game.game_status !==
        "awaiting_trump_suite" ? null : game.your_index === game.whos_turn ? (
          <ChooseTrumpSuiteView />
        ) : (
          <AwaitingTrumpSuiteView />
        )}
        ...
        {game.tricks.length < 1 ? null : (
          <>
            <Divider />
            <Box sx={{ textAlign: "center", py: 2 }}>
              {game.tricks.map(({ starting_player, winner, cards }) => (
                <Box sx={{ mb: 2 }}>
                  {cards.map((card, ix) => (
                    <Box display="inline-block">
                      <Box flexDirection="row">
                        <Box>
                          <Player
                            name={game.players[(starting_player + ix) % 4]}
                            index={((starting_player + ix) % 4) as PlayerIndex}
                            highlighted={winner}
                            lead={game.whos_lead}
                          />
                        </Box>
                        <Box>
                          <Card
                            card={card}
                            highlighted={winner === (starting_player + ix) % 4}
                          />
                        </Box>
                      </Box>
                    </Box>
...
const ResetGamePane: React.FC = () => {
  ...
  if (game.whos_admin !== game.your_index) {
    return (
      <Alert severity="info">
        {t("admin-of-this-game-can-create-the-next-game")}
      </Alert>
    );
  }
  ...
  return (
    <Box>
      <Box sx={{ py: 2 }}>
        <Alert severity="info">
          {t("you-are-the-admin-and-you-can-create-the-next-game")}
        </Alert>
      </Box>
      <Button
        variant="contained"
  ...
  return (
    <CenterContainer>
      <Box textAlign="center" sx={{ mb: 4 }}>
        {players.map((name, ix) => (
          <Player name={name} index={ix as PlayerIndex} />
        ))}
      </Box>
    ...
    <>
      ...
      <Box sx={{ mt: 4 }}>
        <Button
          ...
          disabled={api.pending || invalid_form}
        >
          {t("join-this-game")}
        </Button>
      </Box>
    ...
    <>
      <Box sx={{ my: 4 }}>
        <Alert severity="info">
          {t("waiting-for")}
          <b>
            {4 - game.players.length} {t("more-player")}
          </b>{" "}
          {t("to-join")}
        </Alert>
      </Box>
      <TextField
        label={t("share-this-link-with-your-friends")}
...
const ChooseTrumpSuiteView: React.FC = () => {
  ...
  return (
    <Box sx={{ m: 4 }}>
      <Box sx={{ my: 2 }}>
        <Alert severity="info">{t("please-choose-a-trump-suite")}</Alert>
      </Box>
...
const AwaitingTrumpSuiteView: React.FC = () => {
  ...
  return (
    <Box sx={{ m: 4 }}>
      <Alert severity="info">
        {t("waiting-for")}
        <Player name={players[whos_lead]} index={whos_lead} lead={whos_lead} />
        {t("to-choose-trump-suite")}
      </Alert>
    </Box>
  );
};
...
export const GameScreen: React.FC = () => {
    ...
    return (
      <CenterContainer>
        <Alert severity="error">
          {t("error-in-api-response")} {api.error_code}
        </Alert>
      </CenterContainer>
    );
  }
  ...
  return (
    <>
      ...
      {api.data == null ? null : (
        <GameProvider value={{ ...api.data, game_id: gameId }}>
          <GameView />
        </GameProvider>
      )}
    </>
